...
void main() {
  if (uPaused) {
    // Hold last frame when paused — reuse current uniforms
  }
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    const isPaused = () => !visible || tabHidden;
    ...
    function update(time: number) {
      ...
      /* ── Frame-rate cap: skip if < FRAME_INTERVAL ─────── */
      if (time - lastTime < FRAME_INTERVAL) return;
      lastTime = time;

      /* ── Pause rendering when off-screen ──────────────── */
      const paused = isPaused();
      program!.uniforms.uPaused.value = paused;
      if (paused) return;
